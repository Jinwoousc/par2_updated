    ...
    struct MoveParticle
    {
        ...
        */
        void initialize(thrust::device_vector<T> &_datax,
                        ...
                        thrust::device_vector<T> &_mmb,
                        thrust::device_vector<T> &_mimb1,
                        thrust::device_vector<T> &_mimb2,
                        T _nParticles,
                        ...
                        bool _useTrilinearCorrection)
        {
            ...
            mmb = thrust::raw_pointer_cast(_mmb.data());
        ...
        */
        ...
        using Position = thrust::tuple<T, T, T, T, unsigned int>;
        ...
        */
        ...
        Position operator()(Position p) const
        {
            ...
            if (massType == 1)
            {
                ...
                T nmmb = P100*mmb[id] + P101*mimb1[id];
                T nmimb1 = P110*mmb[id] + P111*mimb1[id];
                // homogeneous masstransfer
                // T nmmb = P100*mmb[0] + P101*mimb1[0];
                // T nmimb1 = P110*mmb[0] + P111*mimb1[0];

                T pmmb = nmmb/(nmmb+nmimb1);

                // apply mass transition probability to particles
                T randomY = curand_uniform_double(&states[thrust::get<4>(p)]);
                ...
                if (thrust::get<3>(p) == 0) // from mobile phase
                {
                    ...
                    else // to immobile phase
                    {
                        ...
                        // heterogeneous masstransfer
                        atomicAdd( &mmb[id], -1.0 );
                        atomicAdd( &mimb1[id], 1.0 );
                        // homogeneous masstransfer
                        // atomicAdd( &mmb[0], -1.0 );
                        // atomicAdd( &mimb1[0], 1.0 );
                        thrust::get<3>(p) = 1;
                    }
                }
            ...
            if (mobilephase)
            {
                ...
                // Random Displacement
                T xi0 = curand_normal_double(&states[thrust::get<4>(p)]);
                T xi1 = curand_normal_double(&states[thrust::get<4>(p)]);
                T xi2 = curand_normal_double(&states[thrust::get<4>(p)]);
