...
namespace par2
{
    ...
    PParticles<T>::PParticles(const grid::Grid<T> &_grid,
                           thrust::host_vector<T> &_mmb,
                           thrust::host_vector<T> &_mimb1,
                           thrust::host_vector<T> &_mimb2,
                                      ...
                                      T p2x, T p2y, T p2z)
    {
        ...
        auto functor = InitVolume<T>(states, grid, mmb, nParticles, p1x, p1y, p1z, p2x, p2y, p2z);
    ...
    void PParticles<T>::exportCSV(const std::string &fileName) const
    {
        ...
        thrust::host_vector<T> hp = cp;

        std::ofstream outStream;
        outStream.open(fileName);
        if (outStream.is_open())
        {
            outStream << "id,x coord,y coord,z coord,mass phase" << std::endl;
            for (unsigned int i = 0; i < nParticles; i++)
            {
                outStream << i << "," << hx[i] << "," << hy[i] << "," << hz[i] << "," << hp[i]
                          << std::endl;
            }
        }
        else
    ...
    {
        // Copy to host memory
        thrust::host_vector<T> hmmb = mmb;
        thrust::host_vector<T> hmimb1 = mimb1;
        thrust::host_vector<T> hmimb2 = mimb2;
        int nt = hmmb.size();
        ...
        if (outStream.is_open())
        {
            outStream << "grid_id, mmb, mimb1, mimb2" << std::endl;
            for (int i = 0; i < nt; i++)
            {
                outStream << i << "," << hmmb[i]/T(nParticles) << "," << hmimb1[i]/T(nParticles) <<  "," << hmimb2[i]/T(nParticles) << std::endl;
            }
        }
        else
        {
